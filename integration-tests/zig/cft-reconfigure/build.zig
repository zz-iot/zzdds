...
// zig/cft-reconfigure -- set_expression_parameters() runtime CFT
// reconfiguration integration test, talking to zzdds's native Zig API
// directly. See docs/design/integration-test-tier.md for the full scenario
// spec. Two separate binaries (publisher/subscriber), matching
// zig/wait-for-historical-data's and zig/enable-defer's convention.
...
pub fn build(b: *std.Build) void {
    ...
    const gen_cft_event = b.addRunArtifact(zidl_exe);
    gen_cft_event.addArgs(&.{ "-b", "zig", "--split-files", "--generate-zzdds-wrappers", "-o" });
    const cft_event_gen_dir = gen_cft_event.addOutputDirectoryArg("cft-event-generated");
    gen_cft_event.addFileArg(b.path("idl/cft_event.idl"));

    const cft_event_gen_mod = b.createModule(.{
        .root_source_file = cft_event_gen_dir.path(b, "cft_event.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
    ...
    const common_imports = &[_]std.Build.Module.Import{
        ...
        .{ .name = "cft_event_gen", .module = cft_event_gen_mod },
        .{ .name = "zidl_rt", .module = zidl_rt_mod },
    };

    const publisher_exe = b.addExecutable(.{
        .name = "cft_reconfigure_publisher",
    ...
    const subscriber_exe = b.addExecutable(.{
        .name = "cft_reconfigure_subscriber",
        .root_module = b.createModule(.{
    ...
    const run_publisher_step = b.step("run-publisher", "Run cft_reconfigure_publisher (pass extra flags via -- ...)");
    ...
    const run_subscriber_step = b.step("run-subscriber", "Run cft_reconfigure_subscriber (pass extra flags via -- ...)");
