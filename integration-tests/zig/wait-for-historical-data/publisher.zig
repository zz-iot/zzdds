//! zig/wait-for-historical-data -- publisher. Talks to zzdds's native Zig
//! API directly. Writes a historical batch immediately, with no reader
//! matched yet, then -- once a reader does match -- writes a live batch.
//! TRANSIENT_LOCAL durability means zzdds's own writer-side cache (not this
//! app) is what makes the historical batch replayable to a late joiner.
//! Direct Zig analogue of examples/zig/catchup/publisher.zig; see
//! docs/design/integration-test-tier.md for the full scenario spec and
//! subscriber.zig for the harder correctness bar this scenario adds over
//! the plain catchup example.
//!
//! Required stdout markers: "Create topic:", "Create writer for topic:",
//! "Publisher: wrote historical seq_num=", "Publisher: reader matched,
//! writing live batch", "Publisher: wrote live seq_num=", "Publisher:
...
pub fn main(init: std.process.Init) !void {
    ...
    const writer = history_event_gen.HistoryEventDataWriter.init(dw, alloc);
