/*
 * integration-tests/cpp/wait-for-historical-data -- publisher. Direct C++
...
int main(int argc, char **argv) {
    ...
    HistoryEventDataWriter writer(dw->native_handle());
    ...
    for (; seq < HISTORICAL_COUNT; seq++) {
        ::HistoryEvent sample;
        sample.seq_num = seq;
        if (writer.write(sample) != ::DDS::RETCODE_OK) {
    ...
    if (state.matched_current_count.load() != 0) {
        std::fprintf(stderr, "FAIL: reader matched before the historical batch finished writing -- not exercising late-join replay\n");
        return 1;
    }
