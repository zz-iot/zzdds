#!/usr/bin/env python3
"""Builds the zzdds Java enable-defer integration test.
...
def main() -> int:
    ...
    print("Generating ConfigEvent TypeSupport/DataWriter/DataReader from idl/config_event.idl...")
    ...
    if not run_build(
        [
            ...
            "-o", str(GENERATED_DIR),
            str(SCRIPT_DIR / "idl" / "config_event.idl"),
        ],
        cwd=SCRIPT_DIR,
    ...
    sources = (
        ...
        + [str(SCRIPT_DIR / "Configurer.java"), str(SCRIPT_DIR / "Peer.java")]
    )
    if not run_build(
