#!/usr/bin/env python3
"""Builds the zzdds Java ignore-entities integration test.

Prerequisites:
  - A zzdds checkout built with `zig build -Djava-binding=true install`
    (set ZZDDS_ZIG_OUT to its zig-out/ dir; defaults to ../../../zig-out).
...
def main() -> int:
    ...
    print("Generating IgnoreEvent TypeSupport/DataWriter/DataReader from idl/ignore_event.idl...")
    ...
    if not run_build(
        [
            ...
            "-o", str(GENERATED_DIR),
            str(SCRIPT_DIR / "idl" / "ignore_event.idl"),
        ],
        cwd=SCRIPT_DIR,
    ...
    sources = (
        ...
        + [str(SCRIPT_DIR / "Ignorer.java"), str(SCRIPT_DIR / "Peer.java"), str(SCRIPT_DIR / "Bystander.java")]
    )
    if not run_build(
