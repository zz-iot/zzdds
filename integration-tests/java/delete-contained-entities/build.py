#!/usr/bin/env python3
"""Builds the zzdds Java delete-contained-entities integration test.
...
def main() -> int:
    ...
    print("Generating SessionEvent TypeSupport/DataWriter/DataReader from idl/session_event.idl...")
    ...
    if not run_build(
        [
            ...
            "-o", str(GENERATED_DIR),
            str(SCRIPT_DIR / "idl" / "session_event.idl"),
        ],
        cwd=SCRIPT_DIR,
    ...
    sources = (
        ...
        + [str(SCRIPT_DIR / "Session.java"), str(SCRIPT_DIR / "Peer.java")]
    )
    if not run_build(
