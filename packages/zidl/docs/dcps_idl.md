# Complete Normative DCPS IDL

Source: OMG DDS v1.4 (formal/15-04-10) §2.3.3.
This is the complete `module DDS { ... }` block, faithfully transcribed.
Also includes the "implied IDL" example (§2.2.2.3.9) for a user type `Foo`.

Implementation reference for `--generate-interfaces`.
...
## Implied IDL for User Type `Foo` (§2.2.2.3.9)

The spec uses a placeholder user type `Foo` to illustrate the typed interfaces
(`FooTypeSupport`, `FooDataWriter`, `FooDataReader`) that code generators must produce.
For a real user type `T`, replace `Foo`/`FooSeq` with the actual type and sequence.
...
- `DataWriter` and `DataReader` use `// commented-out` operations for the type-specific methods.
  Code generators must emit the concrete, typed versions (as shown in the `Foo` implied IDL).

- `TypeSupport::register_type` and `get_type_name` are also commented out in the base interface;
  generators emit them in the concrete `FooTypeSupport`.

- `WaitSet` references `ConditionSeq`: the DDS v1.4 spec §2.3.3 places the typedef *after*
  `WaitSet`, which is technically invalid IDL (typedefs cannot be forward-declared; only
  interfaces can). Real-world implementations all hoist the typedef above `WaitSet`.
  `docs/dcps.idl` follows that corrected ordering.

- The `DomainParticipantFactory` is typically obtained through a language-mapping-specific
  singleton method (not part of the IDL); e.g. `DomainParticipantFactory::get_instance()` in C++.
