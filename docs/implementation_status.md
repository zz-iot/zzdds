...
| TypeSupport C-ABI shim | Complete | `zzdds_register_type_support_c` in `src/c_abi/typesupport.zig`; bridges C `compute_key_hash_from_cdr` function pointer to Zig `TypeSupport` vtable |
...
| C++ binding (`dcps.hpp` + `dcps_impl.hpp/cpp`) | Majority complete | Generated by `zidl -b cpp --generate-interfaces --cpp-generate-impl`; typed DataWriter/DataReader wrappers, listener base classes, CDR, out-param QoS adaptation all done; 11 method stubs remain (see zidl roadmap) |
