...
| Java binding | Majority complete | Generated by `zidl -b java --generate-interfaces --generate-zzdds-wrappers`; real entity JNI bridge (`dcps_jni.c`), full QoS/status struct marshaling, and listener JNI upcalls all done; verified end-to-end with two separate JVM processes discovering, matching, and exchanging data over real UDP RTPS (`zzdds-java-example/`) plus an in-process smoke test (`zig build -Djava-binding=true test-bindings`); a handful of DCPS operations taking a bare `sequence<T>` param directly (not inside a struct) throw `UnsupportedOperationException`; `zzdds.idl`'s vendor extensions aren't available in Java yet (cross-file type references aren't tracked by zidl's Java backend) |
