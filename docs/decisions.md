...
`.random`: 12 OS-entropy bytes. `.host_based`: IP+PID+timestamp — useful for
Wireshark correlation and deterministic tests. The first two bytes must equal the
`VendorId` bytes per RTPS §9.3.1.5.
