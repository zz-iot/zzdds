...
pub const CacheChange = struct {
    ...
    // For locally-produced samples: always CDR2_LE (0x0007 encap ID).
    ...
    data: []u8,  // owned by the history cache allocator
};
```

## Encoding

- **Writer cache**: always CDR2_LE (XCDR2, little-endian). For `@final` types XCDR1 and XCDR2 produce identical bytes; XCDR2 encap ID (`0x0007`) is always used. For `@appendable`/`@mutable` types XCDR2 is the only valid choice.
