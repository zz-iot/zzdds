...
| Historical data | ~~`wait_for_historical_data`~~ — **Done**, Integration-tier `wait-for-historical-data` scenario (see "First-pass classification" below) |
...
| Discovery/ignore | ~~`ignore_participant`/`ignore_topic`/`ignore_publication`/`ignore_subscription`~~ — **Done**, Integration-tier `ignore-entities` scenario (see "First-pass classification" below); `get_discovered_participants` incidentally exercised there too (used to locate the peer's participant handle for `ignore_participant`). Still uncovered: `get_discovered_topics` and the `_data` variants of both getters |
