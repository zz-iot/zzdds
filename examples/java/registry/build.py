#!/usr/bin/env python3
"""Builds the zzdds Java registry example.
...
Prerequisites:
  - A zzdds checkout built with `zig build -Djava-binding=true install`
    ...
    Needs the Sample.instanceState JNI fix -- see
    docs/design/registry-reference-app.md.
...
def main() -> int:
    ...
    print("Generating SensorReading TypeSupport/DataWriter/DataReader from idl/registry_sample.idl...")
    ...
    if not run_build(
        [
            ...
            "-o", str(GENERATED_DIR),
            str(SCRIPT_DIR / "idl" / "registry_sample.idl"),
        ],
        cwd=SCRIPT_DIR,
