#!/usr/bin/env python3
"""Builds the zzdds Java raw-loan example.
...
Prerequisites:
  - A zzdds checkout built with `zig build -Djava-binding=true install`
    ...
    Needs the read-loan JNI fix -- see
    docs/design/raw-loan-reference-app.md ("A real bug found building the
    Java port").
...
def main() -> int:
    ...
    print("Generating LoanedPing TypeSupport/DataWriter/DataReader from idl/loaned_ping.idl...")
    ...
    if not run_build(
        [
            ...
            "-o", str(GENERATED_DIR),
            str(SCRIPT_DIR / "idl" / "loaned_ping.idl"),
        ],
        cwd=SCRIPT_DIR,
