#!/usr/bin/env python3
"""Builds the zzdds Java presence example.
...
Prerequisites:
  - A zzdds checkout built with `zig build -Djava-binding=true install`
    ...
    Needs the LIVELINESS wire fixes -- see
    docs/design/presence-reference-app.md.
...
def main() -> int:
    ...
    print("Generating PresenceBeacon TypeSupport/DataWriter/DataReader from idl/presence_sample.idl...")
    ...
    if not run_build(
        [
            ...
            "-o", str(GENERATED_DIR),
            str(SCRIPT_DIR / "idl" / "presence_sample.idl"),
        ],
        cwd=SCRIPT_DIR,
