...
def build_all(zig_out: Path) -> bool:
    ...
    for bin_ in (
        executable_path(ZIG_DIR / "zig-out" / "bin" / "catchup_pub"),
        executable_path(ZIG_DIR / "zig-out" / "bin" / "catchup_sub"),
        executable_path(C_DIR / "build" / "catchup_pub"),
        executable_path(C_DIR / "build" / "catchup_sub"),
        executable_path(CPP_DIR / "build" / "catchup_pub"),
        executable_path(CPP_DIR / "build" / "catchup_sub"),
    ):
        if not bin_.is_file():
...
def pub_cmd(lang: str, zig_out: Path) -> list[str]:
    return {
        "zig": [str(executable_path(ZIG_DIR / "zig-out" / "bin" / "catchup_pub"))],
        "c": [str(executable_path(C_DIR / "build" / "catchup_pub"))],
        "cpp": [str(executable_path(CPP_DIR / "build" / "catchup_pub"))],
...
def sub_cmd(lang: str, zig_out: Path) -> list[str]:
    return {
        "zig": [str(executable_path(ZIG_DIR / "zig-out" / "bin" / "catchup_sub"))],
        "c": [str(executable_path(C_DIR / "build" / "catchup_sub"))],
        "cpp": [str(executable_path(CPP_DIR / "build" / "catchup_sub"))],
