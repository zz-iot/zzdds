#!/usr/bin/env python3
"""Discovery cross-binding interoperability smoke test: builds all four
discovery ports (zig, c, cpp, java), then runs a representative subset of
...
Each side exercises its own binding's wrapper around the builtin-topic-data
discovery API (get_discovered_topics/get_discovered_topic_data on the
participant, get_matched_subscriptions/get_matched_subscription_data or
get_matched_publications/get_matched_publication_data on the writer/reader)
before falling back to the same minimal reliable pub/sub loop every other
example here uses -- so, like registry, the cross-binding risk is mainly
about each binding's own struct-marshaling of BuiltinTopicData, not a
wire-format-specific concern. A same-binding self-test per language plus a
handful of cross pairs (covering every language as both publisher and
subscriber at least once) covers that.
...
Usage: ZZDDS_ZIG_OUT=/path/to/zzdds/zig-out ./discovery_cross_binding_smoke_test.py
"""
...
ZIG_DIR = REPO_ROOT / "zig" / "discovery"
C_DIR = REPO_ROOT / "c" / "discovery"
CPP_DIR = REPO_ROOT / "cpp" / "discovery"
JAVA_DIR = REPO_ROOT / "java" / "discovery"
...
def build_all(zig_out: Path) -> bool:
    ...
    if not run_build(["zig", "build", "-Doptimize=ReleaseSafe"], cwd=ZIG_DIR, log_path=zig_out_dir / "build.log"):
        print(f"FAIL: zig/discovery build -- see {zig_out_dir}/build.log", file=sys.stderr)
        return False

    for dir_, name in ((C_DIR, "c/discovery"), (CPP_DIR, "cpp/discovery")):
    ...
    if not run_build([sys.executable, str(JAVA_DIR / "build.py")], cwd=JAVA_DIR, log_path=build_log_dir / "build.log", env=env):
        print(f"FAIL: java/discovery build -- see {build_log_dir}/build.log", file=sys.stderr)
        return False

    for bin_ in (
        executable_path(ZIG_DIR / "zig-out" / "bin" / "discovery_pub"),
        executable_path(ZIG_DIR / "zig-out" / "bin" / "discovery_sub"),
        executable_path(C_DIR / "build" / "discovery_pub"),
        executable_path(C_DIR / "build" / "discovery_sub"),
        executable_path(CPP_DIR / "build" / "discovery_pub"),
        executable_path(CPP_DIR / "build" / "discovery_sub"),
    ):
        if not bin_.is_file():
...
def pub_cmd(lang: str, zig_out: Path) -> list[str]:
    return {
        "zig": [str(executable_path(ZIG_DIR / "zig-out" / "bin" / "discovery_pub"))],
        "c": [str(executable_path(C_DIR / "build" / "discovery_pub"))],
        "cpp": [str(executable_path(CPP_DIR / "build" / "discovery_pub"))],
...
def sub_cmd(lang: str, zig_out: Path) -> list[str]:
    return {
        "zig": [str(executable_path(ZIG_DIR / "zig-out" / "bin" / "discovery_sub"))],
        "c": [str(executable_path(C_DIR / "build" / "discovery_sub"))],
        "cpp": [str(executable_path(CPP_DIR / "build" / "discovery_sub"))],
...
def run_pair(pub_lang: str, sub_lang: str, zig_out: Path) -> bool:
    ...
    env = run_env(zig_out)
    logdir = REPO_ROOT / "interop" / ".smoke-logs" / f"discovery-{pub_lang}-{sub_lang}"
    ...
    ok = ok and "Discovery OK (participant):" in pub.log_text()
    ok = ok and "Discovery OK (writer):" in pub.log_text()
    ok = ok and "Publisher: done." in pub.log_text()
    ok = ok and "Discovery OK (reader):" in sub.log_text()
...
def main() -> int:
    ...
    if failed:
        print("FAIL: discovery cross-binding smoke test", file=sys.stderr)
        return 1
    print(f"OK: all {len(PAIRS)} discovery cross-binding pairs (zig, c, cpp, java) interoperate.")
    return 0
