...
pub const Overrides = struct {
    ...
    udp_meta_unicast_port: ?u16 = null,
    udp_data_unicast_port: ?u16 = null,
    udp_meta_multicast_port: ?u16 = null,
    udp_data_multicast_port: ?u16 = null,
...
fn applyEnv(allocator: std.mem.Allocator, cfg: *schema.Config) void {
    ...
    if (envU16("ZZDDS_TRANSPORT_UDP_META_UNICAST_PORT")) |v| cfg.transport.udp.meta_unicast_port = v;
    if (envU16("ZZDDS_TRANSPORT_UDP_DATA_UNICAST_PORT")) |v| cfg.transport.udp.data_unicast_port = v;
    if (envU16("ZZDDS_TRANSPORT_UDP_META_MULTICAST_PORT")) |v| cfg.transport.udp.meta_multicast_port = v;
    if (envU16("ZZDDS_TRANSPORT_UDP_DATA_MULTICAST_PORT")) |v| cfg.transport.udp.data_multicast_port = v;
...
fn applyOverrides(cfg: *schema.Config, ov: Overrides) void {
    ...
    if (ov.udp_meta_unicast_port) |v| cfg.transport.udp.meta_unicast_port = v;
    if (ov.udp_data_unicast_port) |v| cfg.transport.udp.data_unicast_port = v;
    if (ov.udp_meta_multicast_port) |v| cfg.transport.udp.meta_multicast_port = v;
    if (ov.udp_data_multicast_port) |v| cfg.transport.udp.data_multicast_port = v;
